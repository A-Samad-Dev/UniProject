...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const getMenuItems = () => {
    ...
    if (isFacultyHead) {
      return [
        {
          href: "/faculty-head/dashboard",
          label: "Dashboard",
          icon: LayoutDashboard,
        },
        {
          href: "/faculty-head/departments",
          label: "Departments",
          icon: Building2,
        },
        { href: "/faculty-head/lecturers", label: "Lecturers", icon: Users },
        {
          href: "/faculty-head/students",
          label: "Students",
          icon: GraduationCap,
        },
      ];
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 z-30 h-full w-64 bg-white dark:bg-gray-800 shadow-lg transform transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        {/* Logo */}
        <div className="flex items-center justify-between p-4 border-b dark:border-gray-700">
          <Link href="/" className="flex items-center space-x-2">
            <GraduationCap className="w-8 h-8 text-indigo-600 dark:text-indigo-400" />
            <span className="text-xl font-bold bg-linear-to-r from-indigo-600 to-indigo-400 bg-clip-text text-transparent">
              UniSystem
            </span>
          </Link>
          <button
        ...
        <div className="p-4 border-b dark:border-gray-700">
          ...
          <p className="text-xs text-indigo-600 dark:text-indigo-400 mt-1 capitalize">
            {user?.role?.replace("_", " ")}
          </p>
        </div>
